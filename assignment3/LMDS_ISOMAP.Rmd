...
a. Look for a 2-dimensional ($q = 2$) configuration of the data using parameters $k = 5$ and $\tau = 0.05$ in lmds function. Do the scatterplot of the obtained 2-dimensional configuration.
...
That is, in order to understand the meaning of the two retained dimensions, we select 9 representative examples by considering all 9 combinations of low, around 0, and high values of $x_1$ and $x_2$.
...
We perform PCA on our dataset and plot the three possible combinations of principal components.
...
From the plots, we see that increasing values of PC1 correspond to lower radii, thus a narrower zero.
On the other hand, increasing values of PC2 correspond to thicker strokes.
In both cases, a low value of PC3 seems to reduce the influence of PC1 and PC2 and make for "more equal" zeros.
...
We select points for plotting, using the same criterion as above, and plot the corresponding digits.
