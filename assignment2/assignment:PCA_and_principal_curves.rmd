...
plot(pc, col=5, xlim=c(xmin, xmax), ylim=c(ymin,ymax), asp=1)
for (i in 1:5) {
  pts_in_cluster <- which(dendo.4==i)
  pc_scores <- zip_pc$scores[pts_in_cluster,1:2]
  points(pc_scores, col=i)
  #sm.density(pc_scores, col=i, display="slice", add=TRUE)
}
```
...
```{r}
plot(pc$lambda, col=6)
sm.density(pc$lambda, col=2, display="slice", add=TRUE)
```