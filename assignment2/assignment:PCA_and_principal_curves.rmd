...
plot(x=dfs,y=distances,ylab="Total Squared Porojecting Distance",xlab="Degrees of Freedom")
...
LOOCV was applied on the whole dataset, and it will thus simply return the parameter value resulting in the lowest error, which will clearly correspond to a very complex model, ideally one that crosses all the data points. In order to also take into account the models' flatness and generalization ability, one should first split the data into a training and test samples. After fitting the Principal Curve on the training data, the error is evaluated with respect to the projection distance of the new unseen datapoints. At this point, the most likely outcome for the model with df = 50 will be a very low training error, but a very high test error. On the other hand, we would expect the model with the optimal df value to have a comparable performance in both samples.
