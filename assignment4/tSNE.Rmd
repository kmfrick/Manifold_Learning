...
The projection of the previous plot on the ($PC_2, PC_3$) plane is similar to the results of t-SNE, as shown by the following plot.
```{r}
par(mfrow=c(1,2))
plot(conf.Rtsne.zeros.res, xlab = "tSNE1", ylab = "tSNE2")
plot(pc2, pc3)
```

We choose to perform a dimensionality expansion on the t-SNE results and add a third dimension $z = tSNE_1^2 + tSNE_2^2$.
...
The resulting plot is similar to what we observe with PCA, as we can see from the following 3D scatterplot. Both plots have higher density in the lower region and expand on their upper part, and display a slightly curved shape.
